...
def main():
    ...
    cell_style = ParagraphStyle("cell", fontName="CN", fontSize=8, leading=12)
    ...
    for kind, value in parse_markdown(MD.read_text(encoding="utf-8")):
        if kind == "title":
            ...
            table_data = [[Paragraph(esc(cell), cell_style) for cell in row] for row in value]
